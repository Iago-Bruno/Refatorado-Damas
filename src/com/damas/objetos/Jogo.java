...
public class Jogo {
    ...
    // Verifica se o jogador atual está jogando com um peça válida - Refatorado
    private boolean verificarVez(Pedra peca) {
        // Refatorado
        return (peca.pertenceAoJogador(getVez()));
    }
    ...
    private boolean simularMovimentoEValidar(Casa origem, Casa destino) {
        ...
        if ((distanciaX == 2 && distanciaY == 2) && (peca.isTipoValido())) {
            ...
            } else if (peca.getTipo() == Pedra.PEDRA_VERMELHA) {
                    if ((distanciaX == 1 || distanciaY == 1) && (distanciaX == distanciaY) && sentidoY == -1) {
                        return true;
                    } else {
                        return false;
    ...
    private void transformarPedraParaDama(Casa casa) {
        ...
        if (pedra.getTipo() == Peca.PEDRA_BRANCA) {
            Dama dama = new DamaBranca(casa);
            pedra = (Dama) dama;
        } else {
            Dama dama = new DamaVermelha(casa);
            pedra = (Dama) dama;
        }
    }
